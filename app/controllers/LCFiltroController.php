 ...
 * Index Controller
 */
class LCFiltroController extends Controller
{
    /**
     * Process
     */
    ...
    if (Input::post("action") == "listarCarros") {
            ...
            $this->premium();
    } else if (Input::post("action") == "premiumNovo"){
            $this->premiumNovo();
		...
		private function filtroJS()
    {
		 ...
		 foreach($ArrayFiltro as $af){
			 ...
			 if ($linha == 1){
				if ($coluna == "nome"){
					$query .= "(marca LIKE '%$valor%' OR nome LIKE '%$valor%' OR modelo LIKE '%$valor%' OR tipo_carro LIKE '%$valor%' OR base_modelo LIKE '%$valor%')";
				} else if($coluna == "acessorios"){
				 $query .= "(acessorios LIKE '%$valor%'";
...
// 			$this->jsonecho(); 
    ...
    $Carros = Controller::model("Carros");
    $Carros->where(DB::raw("marca LIKE '%$marca%' AND base_modelo LIKE '%$modelo%' "))
