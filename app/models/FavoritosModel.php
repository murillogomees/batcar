 ...
 * 
 */
class FavoritosModel extends DataList
{	
	/**
	 * Initialize
	 */
	...
	{
		$this->setQuery(DB::table(TABLE_PREFIX.TABLE_FAVORITOS));        
	}
    ...
    public function search($search_query)
    {
        ...
        foreach ($search_strings as $sq) {
            ...
            $query->where(function($q) use($sq) {
                $q->Where(TABLE_PREFIX.TABLE_FAVORITOS.".id_carro", "LIKE", "%". $sq ."%");
            });
        }
    ...
    public function fetchData()
    {
        ...
        $this->getQuery()
             ->select(TABLE_PREFIX.TABLE_FAVORITOS.".*");             
