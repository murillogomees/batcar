<?php 
	...
	{	
	    ...
	    public function select($uniqid)
	    {
	    	...
	    	if ($col) {
		    	$query = DB::table(TABLE_PREFIX.TABLE_USUARIOS)
			    	      ->where($col, "=", $uniqid)
			    	      ->limit(1)
			    	      ->select("*");
		    	if ($query->count() == 1) {
	    ...
	    public function insert()
	    {
	    	...
	    	$id = DB::table(TABLE_PREFIX.TABLE_USUARIOS)
		    	->insert(array(
		    		"id" => null,
	    ...
	    public function delete()
	    {
	    	...
	    	DB::table(TABLE_PREFIX.TABLE_USUARIOS)->where("id", "=", $this->get("id"))->delete();
	    	$this->is_available = false;
	    	return true;
	    }
	    ...
	    {
	    	if ($this->isAvailable() && in_array($this->get("account_type"), array("developer", "admin","gerente","supervisor","vendedor"))) {
	    		return true;
	    	}
	    ...
	    {
	    	if ($this->isAvailable() && in_array($this->get("account_type"), array("financeiro", "admin" , "developer","gerente"))) {
	    		return true;
	    	}
	    ...
	    {	
    		...
    		if ($this->get("account_type") == "admin" || $this->get("account_type") == "gerente"){
    			return true;    				
    		}
	    ...
	    {	
    		...
    		if ($this->get("account_type") == "admin" || $this->get("account_type") == "gerente" || $this->get("account_type") == "supervisor"){
    			return true;    				
    		}
